...
# 👇 УПРОЩЁННАЯ ФУНКЦИЯ ДЛЯ ФОТО (как в тестовой версии)
def send_photo_to_telegram(photo_url, sender_name, caption=""):
    """Отправляет фото (упрощённая версия)"""
    try:
        ...
        response = requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto",
        ...
        if history and isinstance(history, list):
            ...
            for msg in reversed(history):
                ...
                # ФОТО (упрощённая версия)
                    ...
                    if download_url:
                        sent = send_photo_to_telegram(download_url, sender_name, caption)
